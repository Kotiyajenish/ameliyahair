...
class WPF_List_Table extends WP_List_Table {
    ...
    function column_title($item) {
        ...
        if(is_null($nonce_export)){
            $nonce_export = wp_create_nonce($this->plugin_name.'_export');
        }
        //Build row actions
        $actions = array(
            'edit' => sprintf(
            ...
            'delete' => sprintf(
                '<a title="%1$s" href="%2$s" class="wpf_delete">%3$s</a>',sprintf(esc_attr__('Delete Product Filter %s','wpf'), esc_attr( $item['name'] ) ), add_query_arg(array('action'=>'wpf_delete','nonce'=>$nonce_delete,'slug'=>$item['slug']),  admin_url('admin-ajax.php')),  __('Delete', 'wpf')
            )
        );
    ...
    function process_bulk_action() {
        ...
        if ('delete' === $this->current_action() && current_user_can('manage_options')) {
            ...
            if ( ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'bulk-wpf_posts' ) ) {
                die;
            }
            ...
            if (isset($_REQUEST[$this->_args['singular']])) {
                ...
                if ($deleted > 0) {
                    ...
                    add_settings_error($this->plugin_name . '_notices', '', esc_html( $message ), 'updated');
                }
            }
        }
    }
