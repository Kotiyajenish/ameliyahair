...
function custom_product_lester_taxonomy(){
	$labels = array(
		...
		'all_items'                  => __('All lester', 'text_domain'),
    ...
    if (isset($_POST['updateuser']) && isset($_POST['action']) && $_POST['action'] === 'change_email') {
        ...
        if (!isset($_POST['_wpnonce']) || !wp_verify_nonce($_POST['_wpnonce'], 'update-user')) {
            ...
            if (!is_email($new_email)) {
				...
				echo '<div class="success email_done">Email updated successfully!</div>';
...
function display_trust_badges_above_order_table() {
    ...
    if ( $current_user->ID ) {
        ...
        echo '<a href="https://devwp1.websiteserverhost.biz/ameliyahair/email-change/" class="reset-email-content"  target="_blank">RESET YOUR EMAIL</a>';
